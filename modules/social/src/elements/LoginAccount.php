...
namespace modules\social\elements;
...
class LoginAccount extends Element
{
    ...
    protected static function defineSources(string $context = null): array
    {
        $sources = [
            [
                'key' => '*',
                'label' => Craft::t('depotise-social', 'All login accounts'),
                'criteria' => [],
                'hasThumbs' => false
            ]
        ];
        ...
        if ($loginProviders) {
            $sources[] = ['heading' => Craft::t('depotise-social', 'Login Providers')];
            ...
            foreach ($loginProviders as $loginProvider) {
                ...
                $sources[] = [
                    'key' => $key,
                    'label' => Craft::t('depotise-social', $loginProvider->getName()),
    ...
    {
        $attributes['socialUid'] = Craft::t('depotise-social', 'Social User ID');
        $attributes['username'] = Craft::t('depotise-social', 'Username');
        $attributes['email'] = Craft::t('depotise-social', 'Email');
        $attributes['providerHandle'] = Craft::t('depotise-social', 'Login Provider');
        $attributes['elements.dateCreated'] = Craft::t('depotise-social', 'Date Created');
        $attributes['elements.dateUpdated'] = Craft::t('depotise-social', 'Date Updated');
        $attributes['lastLoginDate'] = Craft::t('depotise-social', 'Last Login');
    ...
    {
        $attributes['socialUid'] = ['label' => Craft::t('depotise-social', 'Social User ID')];
        $attributes['username'] = ['label' => Craft::t('depotise-social', 'Username')];
        $attributes['fullName'] = ['label' => Craft::t('depotise-social', 'Full Name')];
        $attributes['email'] = ['label' => Craft::t('depotise-social', 'Email')];
        $attributes['loginProvider'] = ['label' => Craft::t('depotise-social', 'Login Provider')];
        $attributes['dateCreated'] = ['label' => Craft::t('depotise-social', 'Date Created')];
        $attributes['dateUpdated'] = ['label' => Craft::t('depotise-social', 'Date Updated')];
        $attributes['lastLoginDate'] = ['label' => Craft::t('depotise-social', 'Last Login')];
     ...
     * Get login provider.
     *
     * @return \modules\social\base\LoginProvider|null
     * @throws \yii\base\InvalidConfigException
     */
    ...
    public function afterSave(bool $isNew)
    {
        if ($isNew) {
            Craft::$app->db->createCommand()
                ->insert('{{%depotise_social_login_accounts}}', [
                    'id' => $this->id,
                    'userId' => $this->userId,
            ...
            Craft::$app->db->createCommand()
                ->update('{{%depotise_social_login_accounts}}', [
                    'userId' => $this->userId,
                    'providerHandle' => $this->providerHandle,
                    'socialUid' => $this->socialUid,
                ], ['id' => $this->id])
